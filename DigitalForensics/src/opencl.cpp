...
int main(int argc, char *argv[]) {
  ...
  cl_kernel kernel = clCreateKernel(program, "search_pattern_opencl_ext", &err);
  ...
  // Copy the node data and text to their respective memory buffers
  ...
  err = clEnqueueWriteBuffer(queue, text_buffer, CL_TRUE, 0,
                             ...
                             nullptr, nullptr);
  // Enqueue commands to write these initial values to the buffers
  ...
  // Read the updated data back to the host for the indices array
  for (int i = 0; i < k; ++i) {
    int offset = i * max_size * sizeof(int);
  ...
  // Output results, cleanup resources
  delete[] vec;
